...
# Хвост: «… ещё N», сливается в одну строку со сводкой ниже (одна ссылка на полный файл)
tail_line=""
if [ "$total" -gt "$LIMIT" ]; then
  tail_line="… and $((total - LIMIT)) more with ${STARS_LABEL} stars · "
fi

BLOCK="$list
$tail_line**$total merged pull requests** to external projects with $STARS_LABEL stars · [full list of all $full_total]($LIST_FILE) · [on GitHub]($search_url)"
